"""
独立微调脚本 - 修复版
关键修复: preprocess_function 中不使用 return_tensors="pt"
"""
import os
import sys
import json
...
class AnnualReportTrainer:
    ...
    def load_model_and_tokenizer(self):
        ...
        logger.info(f"Model loaded. Total: {total/1e9:.2f}B, Trainable: {trainable/1e6:.2f}M")
    ...
    def train(self):
        # Step 1: Load tokenizer only and tokenize data (before model to save RAM)
        ...
        # Step 2: Now load model (tokenizer cache can be freed)
...
def main():
    ...
    logger.info(f"Val file: {config.validation_file}")
    ...
    logger.info(f"4-bit: {config.load_in_4bit}, FP16: {config.fp16}")
